...
// testflight archives must be signed during archive, not left unsigned for export to repair
// export can still upload an IPA from an unsigned archive, but the final app may only get
// minimal App Store entitlements and lose iCloud, NFC, and associated domains
// the entitlement validator below fails before upload if this regresses
const IOS_TESTFLIGHT_CODE_SIGN_IDENTITY: &str = "Apple Distribution";
...
fn validate_testflight_archive_entitlements(sh: &Shell, archive_path: &str) -> Result<()> {
    ...
    let stdout =
        String::from_utf8(output.stdout).wrap_err("Failed to parse codesign entitlement stdout")?;
    let stderr =
        String::from_utf8(output.stderr).wrap_err("Failed to parse codesign entitlement stderr")?;

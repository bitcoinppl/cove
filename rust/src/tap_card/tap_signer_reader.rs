...
impl ContinuationClaim {
    fn try_new(claimed: &Arc<AtomicBool>) -> Option<Self> {
        claimed
            .compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
            .then(|| Self { claimed: Some(Arc::clone(claimed)) })
    }
...
impl Drop for ContinuationClaim {
    fn drop(&mut self) {
...
impl TapSignerSetupContinuation {
    ...
    fn claim(&self) -> Option<ContinuationClaim> {
        ContinuationClaim::try_new(&self.claimed)
    }
...
impl TapSignerReader {
    ...
    async fn continue_setup(
        ...
        let claim =
            continuation.claim().ok_or(TapSignerReaderError::SetupContinuationAlreadyUsed)?;

        let result = match continuation.stage() {
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl TapcardTransportProtocol for PendingTransport {
        fn set_message(&self, _message: String) {}

        fn append_message(&self, _message: String) {}

        async fn transmit_apdu(&self, _command_apdu: Vec<u8>) -> Result<Vec<u8>, TransportError> {
    ...
    fn reader_for_responses_with_identity(
        ...
        let reader = reader_with_transport(transport, path, num_backups, card_identity);

        (reader, calls)
    }

    fn reader_with_transport(
        transport: TapcardTransport,
        path: Option<Vec<u32>>,
        num_backups: Option<u32>,
        card_identity: String,
    ...
    fn reader_for_pending_transport(started: Arc<AtomicBool>) -> TapSignerReader {
        let transport = TapcardTransport(Arc::new(Box::new(PendingTransport { started })));
        reader_with_transport(transport, Some(vec![84, 0, 0]), Some(0), "TEST-CARD".to_string())
    }
    ...
    #[tokio::test]
    async fn dropping_setup_continuation_future_releases_claim() {
        let started = Arc::new(AtomicBool::new(false));
        let reader = reader_for_pending_transport(started.clone());
        let cvc = Arc::new(TapSignerCvc::try_new("123456".to_string()).unwrap());
        let setup = Arc::new(SetupCmd::try_new(cvc.clone(), cvc, Some([0; 32].to_vec())).unwrap());
        let continuation = TapSignerSetupContinuation::new(
            "TEST-CARD".to_string(),
        ...
        let task = tokio::spawn(async move { reader.continue_setup(task_continuation).await });
        ...
        let claim = continuation.claim().expect("canceled setup continuation is retryable");
        claim.consume();
    }
    ...
    #[tokio::test]
    async fn dropping_operation_continuation_future_releases_claim() {
        let started = Arc::new(AtomicBool::new(false));
        let reader = reader_for_pending_transport(started.clone());
        let cvc = Arc::new(TapSignerCvc::try_new("123456".to_string()).unwrap());
        let continuation = TapSignerOperationContinuation::new(
        ...
        let task_continuation = continuation.clone();
        let task = tokio::spawn(async move { reader.continue_operation(task_continuation).await });
    ...
    fn resolver_routes_matching_command_and_blocks_mismatched_operations() {
        ...
        let claim = pending.claim().expect("pending continuation is unclaimed");
        claim.consume();
        assert_eq!(

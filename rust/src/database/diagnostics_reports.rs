...
use redb::{ReadableTable as _, TableDefinition, TypeName, Value};
...
impl Value for DiagnosticsReportsJson {
    ...
    where
        Self: 'b,
    {
        serde_json::to_vec(value).expect("failed to serialize diagnostics report history")
    }
...
impl DiagnosticsReportsTable {
    ...
    pub fn add(&self, record: DiagnosticsReportRecord) -> Result<(), Error> {
...

        {
            ...
            table.insert(HISTORY_KEY, &records).map_err_str(DiagnosticsReportsTableError::Save)?;
        }

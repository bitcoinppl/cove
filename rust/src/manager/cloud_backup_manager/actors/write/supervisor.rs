...
//!
//! This actor serializes cloud writes, enforces destructive-operation fences,
...
mod tests {
    ...
    #[test]
    fn drive_account_switch_fence_only_allows_transition_owned_writes() {
        let transition_id = 7;
        let blocker = CloudBackupWriteBlocker::DriveAccountSwitch { transition_id };
        let owner = CloudBackupExclusiveOperationClaim::new(
            CloudBackupExclusiveOperation::ReinitializeBackup,
            transition_id,
        );
        let background = CloudBackupWriteCommandContext::new(1, None);
        let owned = CloudBackupWriteCommandContext::new(2, Some(owner));
        let stale = CloudBackupWriteCommandContext::new(
            3,
        ...
        assert!(CloudBackupWriteAdmissionCheck::new(Some(blocker)).writes_allowed(owned).is_ok());
        assert!(matches!(
            CloudBackupWriteAdmissionCheck::new(Some(blocker)).writes_allowed(background),
            Err(CloudBackupError::Deferred(_))
        ));
        assert!(matches!(
            CloudBackupWriteAdmissionCheck::new(Some(blocker)).writes_allowed(stale),
            Err(CloudBackupError::Deferred(_))
        ));
        assert!(matches!(
            CloudBackupWriteAdmissionCheck::new(Some(blocker)).writes_allowed(wrong_operation),
            Err(CloudBackupError::Deferred(_))
        ));
    }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn drive_account_switch_block_receiver_waits_for_in_flight_write() {
        let mut supervisor = CloudBackupWriteSupervisor::default();
        let blocker = CloudBackupWriteBlocker::DriveAccountSwitch { transition_id: 7 };
        supervisor.in_flight_write =
            Some(in_flight_write(0, CloudBackupWriteAdmission::RequiresWritesAllowed));

        let receiver = supervisor.block_until_drained_receiver(blocker).await.unwrap();

        assert_eq!(supervisor.active_blocker, Some(blocker));
        assert_eq!(supervisor.drain_receivers.len(), 1);
        assert!(
            supervisor
                .in_flight_write
                .as_ref()
        ...
        assert!(supervisor.in_flight_write.is_none());
        assert!(supervisor.drain_receivers.is_empty());
    }

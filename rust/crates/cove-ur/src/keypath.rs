...
impl CryptoKeypath {
    ...
    pub fn from_cbor(cbor: &[u8]) -> Result<Self> {
        minicbor::decode(cbor).map_err(|e| UrError::CborDecodeError(e.to_string()))
    }
}
...
mod tests {
    ...
    fn test_keypath_pair_format_decode() {
        ...
        assert_eq!(
            decoded.components,
            vec![0x80000054, 0x80000000, 0x80000000]
        );
    ...
    fn test_keypath_encodes_integer_format() {
        let keypath =
            CryptoKeypath::new(vec![0x80000054, 0x80000000, 0x80000000], None, None);

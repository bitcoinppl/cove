...
**Manager ownership and cleanup:** Managers obtained via `app.getWalletManager()` or `app.getSendFlowManager()` are owned by `AppManager`—components should not call `.close()` on them or clear them from route-level `DisposableEffect` cleanup. Compose navigation can keep an outgoing route alive while the incoming route has already reused the same cached manager, so route disposal is not the same thing as the flow ending. App-owned managers should be released by the app/session owner when the full route stack no longer contains the owning flow or wallet. Only close managers created locally (e.g., `CoinControlManager`, `ImportWalletManager`, `TapSignerManager`). For short-lived managers like `LabelManager`, use `.use { }` for single operations or `DisposableEffect` with `.close()` if stored in state. `Database()` returns an Arc clone of a global singleton and doesn't need closing.

**Generated manager handles:** Platform UI should call Kotlin/Swift manager wrapper methods instead of reaching into generated UniFFI objects such as `manager.rust.*`. Android managers should keep generated handles private, keep constructors that receive them internal, and route Rust access through wrapper methods backed by the shared `RustHandleGuard`, so close-after-dispose races become safe no-ops or controlled failures without scattering lifecycle checks through call sites.

...
**Generated manager handles:** Platform UI should call Kotlin/Swift manager wrapper methods instead of reaching into generated UniFFI objects such as `manager.rust.*`. The platform manager is responsible for checking whether its generated handle is still open, translating close-after-dispose races into safe no-ops or controlled failures, and preserving the manager ownership boundary described above.

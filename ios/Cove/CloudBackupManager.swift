...
final class CloudBackupManager: ReconcilingManager, CloudBackupManagerReconciler, @unchecked Sendable {
    ...
    var undecryptableWalletDeletionState: CloudBackupUndecryptableWalletDeletionState {
        configuredState?.undecryptableWalletDeletion ?? .idle
    }

    var isDeletingUndecryptableWalletBackups: Bool {
        if case .deleting = undecryptableWalletDeletionState { return true }
        return false
    }

    var isPerformingDestructiveAction: Bool {
        destructiveOperationState != .idle || isDeletingUndecryptableWalletBackups
    }

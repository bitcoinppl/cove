...
extension MainSettingsAlertState: TaggedAlertPresentable {
    func alert(context: MainSettingsPresentationContext) -> AnyAlertBuilder {
        ...
        case let .unverifiedWallets(walletId):
            unverifiedWalletsAlert(walletId: walletId, context: context)

        case .confirmEnableWipeMePin:
            confirmEnableWipeDataPinAlert(context: context)

        case .confirmDecoyPin:
            confirmDecoyPinAlert(context: context)

        case .notePinRequired:
            notePinRequiredAlert(context: context)

        case let .noteFaceIdDisabling(nextAlertState):
            noteFaceIdDisablingAlert(next: nextAlertState, context: context)

        case .noteNoFaceIdWhenTrickPins:
            noFaceIdWithTrickPinsAlert(context: context)

        case .noteNoFaceIdWhenWipeMePin:
            noFaceIdWithWipeDataPinAlert(context: context)

        case .noteNoFaceIdWhenDecoyPin:
            noFaceIdWithDecoyPinAlert(context: context)

        case .confirmBetaImportExport:
            confirmBetaImportExportAlert(context: context)

        case let .extraSetPinError(error):
            extraSetPinErrorAlert(error: error, context: context)
        }
    }
}

private func unverifiedWalletsAlert(
    walletId: WalletId,
...
private func confirmEnableWipeDataPinAlert(
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
        title: "Are you sure?",
...
private func confirmDecoyPinAlert(context: MainSettingsPresentationContext) -> AnyAlertBuilder {
    AlertBuilder(
        title: "Are you sure?",
        message:
        """
...
private func notePinRequiredAlert(context: MainSettingsPresentationContext) -> AnyAlertBuilder {
    AlertBuilder(
...
private func noteFaceIdDisablingAlert(
    next: MainSettingsAlertState,
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
...
private func noFaceIdWithTrickPinsAlert(
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
        title: "Can't do that",
...
private func noFaceIdWithWipeDataPinAlert(
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
        title: "Can't do that",
        ...
        actions: {
            ...
            Button("Disable Wipe Data PIN", role: .destructive) {
                if !context.auth.isDecoyPinEnabled {
                    let next = TaggedItem(MainSettingsSheetState.enableBiometric)
                    context.presentSheet(.removeWipeDataPin(next))
                } else {
...
private func noFaceIdWithDecoyPinAlert(
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
        title: "Can't do that",
        ...
        actions: {
            ...
            Button("Disable Decoy Pin", role: .destructive) {
                if !context.auth.isWipeDataPinEnabled {
                    let next = TaggedItem(MainSettingsSheetState.enableBiometric)
                    context.presentSheet(.removeDecoyPin(next))
                } else {
...
private func confirmBetaImportExportAlert(
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(
...
private func extraSetPinErrorAlert(
    error: String,
    context: MainSettingsPresentationContext
) -> AnyAlertBuilder {
    AlertBuilder(

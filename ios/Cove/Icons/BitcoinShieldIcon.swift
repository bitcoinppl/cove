...
struct BitcoinShieldIcon: View {
    ...
    struct ShieldShape: Shape {
        func path(in _: CGRect) -> Path {
            var path = Path()
            Self.addOuterShield(to: &path)
            Self.addInnerShield(to: &path)
            return path
        }

        private static func addOuterShield(to path: inout Path) {
        ...
        private static func addInnerShield(to path: inout Path) {
    ...
    struct BitcoinShape: Shape {
        func path(in _: CGRect) -> Path {
            var path = Path()
            Self.addOutlineStart(to: &path)
            Self.addOutlineEnd(to: &path)
            Self.addUpperCounter(to: &path)
            Self.addLowerCounter(to: &path)
            return path
        }

        private static func addOutlineStart(to path: inout Path) {
        ...
        private static func addOutlineEnd(to path: inout Path) {
            path.addCurve(
        ...
        private static func addUpperCounter(to path: inout Path) {
        ...
        private static func addLowerCounter(to path: inout Path) {
